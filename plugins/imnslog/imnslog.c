...
initConfigSettings(void)
{
	...
	cs.pszPath = NULL;
	cs.iFacilIntMsg = nslog_FacilIntMsg();
}
...
/* log an imnslog-internal message
 * rgerhards, 2008-04-14
 */
rsRetVal imnslogLogIntMsg(syslog_pri_t priority, const char *fmt, ...)
{
	DEFiRet;
	va_list ap;
 ...
 * rgerhards, 2008-04-14
 */
rsRetVal nslog_syslog(modConfData_t *pModConf, syslog_pri_t priority, uchar *pMsg, struct timeval *tp)
{
	syslog_pri_t pri;
	int bPRISet = 0;
	rsRetVal localRet;
...
static rsRetVal runInput(thrdInfo_t __attribute__((unused)) *pThrd)
{
	...
	while(!pThrd->bShallStop) {
		/* nslog_main() waits for the next kernel message, obtains it
		 * and then submits it to the rsyslog main queue.
		 * rgerhards, 2008-04-09
		 */
		CHKiRet(nslog_main(runModConf));
	}
finalize_it:
	RETiRet;
}
...
static rsRetVal beginCnfLoad(modConfData_t **ptr, __attribute__((unused)) rsconf_t *pConf)
{
	...
	pModConf->bKeepKernelStamp = 0;
	pModConf->iFacilIntMsg = nslog_FacilIntMsg();
...
static rsRetVal activateCnfPrePrivDrop(modConfData_t *ptr)
{
	...
	runModConf = pModConf;
	iRet = nslog_WillRunPrePrivDrop(runModConf);
	RETiRet;
}
...
{
	DEFiRet;
	iRet = nslog_WillRunPostPrivDrop(runModConf);
	RETiRet;
}
...
static rsRetVal afterRun(void)
{
	...
	iRet = nslog_AfterRun(runModConf);
	RETiRet;
}
...
rsRetVal __attribute__((unused)) modInit(int iIFVersRequested __attribute__((unused)), 
...
modInfo_t __attribute__((unused)) *pModInfo)
{
	...
	*ipIFVersProvided = CURR_MOD_IF_VERSION; /* we only support the current interface specification */
	...
	CHKiRet(prop.CreateStringProp(&pInputName, UCHAR_CONSTANT("imnslog"), sizeof("imnslog") - 1));
	...
	CHKiRet(regCfSysLineHdlr2((uchar *)"nslogpath", 0, eCmdHdlrGetWord,
			NULL, &cs.pszPath, STD_LOADABLE_MODULE_ID, &bLegacyCnfModGlobalsPermitted));
	CHKiRet(omsdRegCFSLineHdlr((uchar *)"nslogsymbollookup", 0, eCmdHdlrGoneAway,
			NULL, NULL, STD_LOADABLE_MODULE_ID));
	CHKiRet(omsdRegCFSLineHdlr((uchar *)"nslogsymbolstwice", 0, eCmdHdlrGoneAway,
			NULL, NULL, STD_LOADABLE_MODULE_ID));
	CHKiRet(omsdRegCFSLineHdlr((uchar *)"nslogusesyscallinterface", 0, eCmdHdlrGoneAway,
			NULL, NULL, STD_LOADABLE_MODULE_ID));
	CHKiRet(regCfSysLineHdlr2((uchar *)"nslogpermitnonkernelfacility", 0, eCmdHdlrBinary,
			NULL, &cs.bPermitNonKernel, STD_LOADABLE_MODULE_ID, &bLegacyCnfModGlobalsPermitted));
	CHKiRet(regCfSysLineHdlr2((uchar *)"nslogconsoleloglevel", 0, eCmdHdlrInt,
			NULL, &cs.console_log_level, STD_LOADABLE_MODULE_ID, &bLegacyCnfModGlobalsPermitted));
	CHKiRet(regCfSysLineHdlr2((uchar *)"nsloginternalmsgfacility", 0, eCmdHdlrFacility,
			NULL, &cs.iFacilIntMsg, STD_LOADABLE_MODULE_ID, &bLegacyCnfModGlobalsPermitted));
	CHKiRet(regCfSysLineHdlr2((uchar *)"nsklogparsekerneltimestamp", 0, eCmdHdlrBinary,
